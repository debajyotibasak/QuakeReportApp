...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            //which represents the list of features or earthquake
            JSONArray earthquakeArray = baseJsonResponse.getJSONArray("features");

            //For each earthquake in the earthquake array, create an earthquake object
            for (int i = 0; i < earthquakeArray.length(); i++) {

                //get the current element in the i postition of earthquake array
                JSONObject currentEarthquake = earthquakeArray.getJSONObject(i);

                //From the current earthquake we get the properties
                JSONObject properties = currentEarthquake.getJSONObject("properties");

                //we successfully get the magnitude, location, time and url from the current earthquake
                double magnitude = properties.getDouble("mag");
                String location = properties.getString("place");
                long time = properties.getLong("time");
                String url = properties.getString("url");

                //we make a new element for the earthquakes array with the help the string we extract
                //we add the elememt to the array
